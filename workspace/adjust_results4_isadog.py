...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    with open(dogfile, "r") as dog_file:
        file_line = dog_file.readline()
        
        while file_line != "":
            file_line = file_line.rstrip()
            if file_line in dog_name_dict:
                dog_name_dict.update({file_line: 1})
            else:
                dog_name_dict[file_line] = 1
            file_line = dog_file.readline()    

        # Check to see if results_dic items are dogs
        for key in results_dic:
            if results_dic[key][0] in dog_name_dict:
